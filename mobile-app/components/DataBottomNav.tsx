...
type NavRoute = 'BarGraph' | 'WeeklySpending' | 'Description' | 'Scan' | 'ViewReceipt';
...
type NavHref = '/bar-graph' | '/weekly-spending' | '/description' | '/scan' | '/view-receipt';

function toHref(route: NavRoute): NavHref {
  switch (route) {
    case 'BarGraph':
      return '/bar-graph';
    case 'WeeklySpending':
...
export function DataBottomNav({ active }: Props) {
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <Pressable key={item.label} style={styles.item} onPress={() => router.push(toHref(item.route))} hitSlop={6}>
